...
void W::MainWindow(IDirect3DDevice9* pDevice)
{
	...
	if (!I::Engine->IsTakingScreenshot() && !I::Engine->IsDrawingLoadingImage())
	{
		...
		ImGui::BeginMainMenuBar();
		{
			...
			//	ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), XorStr("insecure (development mode)"));

			if (I::Engine->IsInGame())
				ImGui::TextColored(G::bSendPacket ? ImVec4(0.0f, 1.0f, 0.0f, 1.0f) : ImVec4(1.0f, 0.0f, 0.0f, 1.0f), XorStr("choke"));

			const char* const szName = XorStr("fl0whaxx | " __DATE__);
	...
	if (bMainOpened)
	{
		...
		ImGui::Begin(XorStr("fl0whaxx"), &bMainOpened, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse);
		{
...
void T::Visuals()
{
				...
				CTab{ "main", [&style]()
			{
				...
				if (C::Get<bool>(Vars.bEspMainEnemies) || C::Get<bool>(Vars.bEspMainAllies))
				{
					...
					if (C::Get<bool>(Vars.bEspMainPlayerInfo))
					{
						...
						//ImGui::Checkbox(XorStr("skeleton##player"), &C::Get<bool>(Vars.bEspMainSkeleton));
					}
				}
	...
	ImGui::NextColumn();
	{
		...
		ImGui::BeginChild(XorStr("visuals.screen"), ImVec2(0, 0), true, ImGuiWindowFlags_MenuBar);
		{
			...
			ImGui::Separator();
			ImGui::Checkbox(XorStr("predict grenade"), &C::Get<bool>(Vars.bMiscPredictNade));
			ImGui::Checkbox(XorStr("flip knife view"), &C::Get<bool>(Vars.bMiscRightHandKNF));
...
void T::Miscellaneous()
{
	...
	ImGui::Columns(2, nullptr, false);
	{
		...
		ImGui::BeginChild(XorStr("misc.movement"), ImVec2(0, flMovementChildSize), true, ImGuiWindowFlags_MenuBar);
		{
			...
			ImGui::Checkbox(XorStr("custom clantag"), &C::Get<bool>(Vars.bMiscClanTag));
			ImGui::PopStyleVar();
		...
		ImGui::BeginChild(XorStr("misc.exploits"), ImVec2(0, 0), true, ImGuiWindowFlags_MenuBar);
		{
			...
			ImGui::Checkbox(XorStr("fake prime"), &C::Get<bool>(Vars.bMiscFakePrime));
			ImGui::Checkbox(XorStr("disable UI blur"), &C::Get<bool>(Vars.bMiscDisableBlur));
			ImGui::SliderInt(XorStr("strength"), &C::Get<int>(Vars.iMiscRagdollGravity), 0, 600, "%d%%");
			ImGui::Checkbox(XorStr("inverse ragdoll gravity"), &C::Get<bool>(Vars.bMiscInverseGravity));
			ImGui::PopStyleVar();
