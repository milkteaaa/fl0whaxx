...
bool CVisuals::Chams(CBaseEntity* pLocal, DrawModelResults_t* pResults, const DrawModelInfo_t& info, matrix3x4_t* pBoneToWorld, float* flFlexWeights, float* flFlexDelayedWeights, const Vector& vecModelOrigin, int nFlags)
{
	...
	{
		std::make_pair(CreateMaterial(XorStr("fl0whaxx_players"), XorStr("VertexLitGeneric")),
		CreateMaterial(XorStr("fl0whaxx_players_flat"), XorStr("UnlitGeneric"))),

		std::make_pair(CreateMaterial(XorStr("fl0whaxx_viewmodel"), XorStr("VertexLitGeneric")),
		CreateMaterial(XorStr("fl0whaxx_viewmodel_flat"), XorStr("UnlitGeneric"))),

		std::make_pair(CreateMaterial(XorStr("fl0whaxx_reflective"), XorStr("VertexLitGeneric"), XorStr("vgui/white"), XorStr("env_cubemap")),
		CreateMaterial(XorStr("fl0whaxx_glow"), XorStr("VertexLitGeneric"), XorStr("vgui/white"), XorStr("models/effects/cube_white"))),

		std::make_pair(CreateMaterial(XorStr("fl0whaxx_scroll"), XorStr("VertexLitGeneric"), XorStr("dev/screenhighlight_pulse"), "", false, false, szScrollProxies),
...
void CVisuals::Bomb(const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame)
{
	...
	// frame
	D::AddRect(ImVec2(ctx.box.left - 1, ctx.box.top), ImVec2(ctx.box.right + 1, ctx.box.bottom), colFrame, IMGUI_RECT_FILLED, Color(0, 0, 0, 255), 5.0f);
	// icon
...
void CVisuals::PlantedBomb(CPlantedC4* pBomb, float flServerTime, const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame, const Color& colDefuse, const Color& colFailDefuse, const Color& colBackground, const Color& colOutline)
{
	...
	/* info */
	// frame
	D::AddRect(ImVec2(ctx.box.left - 1, ctx.box.top), ImVec2(ctx.box.right + 1, ctx.box.bottom), colFrame, IMGUI_RECT_FILLED, Color(0, 0, 0, 255), 5.0f, ImDrawCornerFlags_Top);
	// icon
	...
	/* timer bar */
	// background
	D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2), ImVec2(ctx.box.right, ctx.box.bottom + 4), colBackground, IMGUI_RECT_FILLED | IMGUI_RECT_OUTLINE, colOutline);
	// bar
	D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2), ImVec2(ctx.box.left + ctx.box.width * flFactor, ctx.box.bottom + 4), Color::FromHSB(flHue, 1.f, 1.f), IMGUI_RECT_FILLED);
	...
	if (pDefuser != nullptr)
	{
		...
		/* defusing bar */
		// background
		D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), ImVec2(ctx.box.right, ctx.box.bottom + 4 + ctx.arrPadding.at(DIR_BOTTOM)), colBackground, IMGUI_RECT_FILLED | IMGUI_RECT_OUTLINE, colOutline);
		// bar
		D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), ImVec2(ctx.box.left + ctx.box.width * flDefuseFactor, ctx.box.bottom + 4 + ctx.arrPadding.at(DIR_BOTTOM)), (flDefuseTime < flCurrentTime) ? colDefuse : colFailDefuse, IMGUI_RECT_FILLED);
	}
}
...
void CVisuals::Grenade(CBaseEntity* pGrenade, EClassIndex nIndex, float flServerTime, const Vector2D& vecScreen, Context_t& ctx, const Color& colFrame, const Color& colBackground, const Color& colOutline)
{
	...
	if (flFactor > 0.f)
	{
		...
		D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2), ImVec2(ctx.box.right, ctx.box.bottom + 4), colBackground, IMGUI_RECT_FILLED | IMGUI_RECT_OUTLINE, colOutline);
		// bar
		D::AddRect(ImVec2(ctx.box.left, ctx.box.bottom + 2), ImVec2(ctx.box.left + ctx.box.width * flFactor, ctx.box.bottom + 4), colGrenade, IMGUI_RECT_FILLED);
	}
}
...
void CVisuals::DroppedWeapons(CBaseCombatWeapon* pWeapon, short nItemDefinitionIndex, Context_t& ctx, const Color& colPrimary, const Color& colAmmo, const Color& colBackground, const Color& colOutline)
{
	...
	if (C::Get<bool>(Vars.bEspMainWeaponIcon))
	{
		...
		D::AddText(F::Icons, 10.f, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecNameSize.x * 0.5f, ctx.box.bottom + 3), szIcon, colPrimary, IMGUI_TEXT_OUTLINE, colOutline);
	...
	if (C::Get<bool>(Vars.bEspMainWeaponDistance))
	{
		...
		D::AddText(F::SmallestPixel, 12.f, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), szDistance, colPrimary, IMGUI_TEXT_OUTLINE, colOutline);
...
void CVisuals::Player(CBaseEntity* pLocal, CBaseEntity* pEntity, Context_t& ctx, const Color& colInfo, const Color& colFrame, const Color& colOutline)
{
	...
	if (C::Get<bool>(Vars.bEspMainSkeleton))
	{
		//CVisuals::Skeleton(pEntity);		
	}
	...
	if (C::Get<bool>(Vars.bEspMainPlayerName))
	{
		...
		if (pInfo.bFakePlayer)
		{
			...
			D::AddText(F::SmallestPixel, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f + 1 + vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecBotSize.y - ctx.arrPadding.at(DIR_TOP)), szBot, Color(140, 140, 140), IMGUI_TEXT_OUTLINE, colOutline);
		}

		D::AddText(F::SmallestPixel, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecNameSize.x * 0.5f - vecBotSize.x * 0.5f, ctx.box.top - 2 - vecNameSize.y - ctx.arrPadding.at(DIR_TOP)), szName, colInfo, IMGUI_TEXT_OUTLINE, colOutline);
		...
		if (C::Get<bool>(Vars.bEspMainPlayerWeapons))
		{
			if (const auto hWeapons = pEntity->GetWeaponsHandle(); hWeapons != nullptr)
			{
				...
				for (int nIndex = MAX_WEAPONS - 1; hWeapons[nIndex]; nIndex--)
				{
					...
					D::AddText(F::Icons, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecIconSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), szIcon, pCurrentWeapon == pActiveWeapon ? colInfo : Color(160, 160, 160), IMGUI_TEXT_OUTLINE, colOutline);
	...
	if (C::Get<bool>(Vars.bEspMainPlayerDistance))
	{
		...
		D::AddText(F::SmallestPixel, flFontSize, ImVec2(ctx.box.left + ctx.box.width * 0.5f - vecDistanceSize.x * 0.5f, ctx.box.bottom + 2 + ctx.arrPadding.at(DIR_BOTTOM)), szDistance, colInfo, IMGUI_TEXT_OUTLINE, colOutline);
		ctx.arrPadding.at(DIR_BOTTOM) += vecDistanceSize.y;
	}
	#pragma endregion
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_HELMET) && pEntity->HasHelmet())
	{
		...
		D::AddText(F::Icons, flFontSize, ImVec2(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), szHelmetIcon, colInfo, IMGUI_TEXT_OUTLINE, colOutline);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_KEVLAR) && pEntity->GetArmor() > 0)
	{
		...
		D::AddText(F::Icons, flFontSize, ImVec2(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), szKevlarIcon, colInfo, IMGUI_TEXT_OUTLINE, colOutline);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_KIT) && pEntity->HasDefuser())
	{
		...
		D::AddText(F::Icons, flFontSize, ImVec2(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), szKitIcon, pEntity->IsDefusing() ? Color(80, 180, 200) : colInfo, IMGUI_TEXT_OUTLINE, colOutline);
	...
	if (C::Get<std::vector<bool>>(Vars.vecEspMainPlayerFlags).at(INFO_FLAG_ZOOM) && pEntity->IsScoped())
	{
		...
		D::AddText(F::Icons, flFontSize, ImVec2(ctx.box.right + 2, ctx.box.top + ctx.arrPadding.at(DIR_RIGHT)), szTargetIcon, colInfo, IMGUI_TEXT_OUTLINE, colOutline);
...
void CVisuals::HealthBar(Context_t& ctx, const float flFactor, const Color& colPrimary, const Color& colBackground, const Color& colOutline)
{
	...
	// bar
	D::AddRect(ImVec2(ctx.box.left - 5 - ctx.arrPadding.at(DIR_LEFT), ctx.box.bottom - (ctx.box.height * flFactor)), ImVec2(ctx.box.left - 3 - ctx.arrPadding.at(DIR_LEFT), ctx.box.bottom), colPrimary, IMGUI_RECT_FILLED);
...
/*
void CVisuals::CreateLine(float x1, float y1, float x2, float y2, Color color, float size)
{
	g_EngineSurface->DrawSetColor(color);

	if (size == 1.f)
		g_EngineSurface->DrawLine(x1, y1, x2, y2);
	else
		g_EngineSurface->DrawFilledRect(x1 - (size / 2.f), y1 - (size / 2.f), x2 + (size / 2.f), y2 + (size / 2.f));
}

void CVisuals::Skeleton(CBaseEntity* pEntity)
{
	auto aModel = pEntity->GetModel();
	if (pEntity && !aModel)
		return;
	studiohdr_t* pStudioModel = g_MdlInfo->GetStudioModel(aModel);	
	if (pStudioModel) {
		static matrix3x4_t pBoneToWorldOut[128];

		if (pEntity->SetupBones(pBoneToWorldOut, 128, 256, g_GlobalVars->flCurrentTime))
		{
			for (int i = 0; i < pStudioModel->nBones; i++)
			{
				mstudiobone_t* pBone = pStudioModel->GetBone(i);
				if (!pBone || !(pBone->iFlags * 256) || pBone->iParent == -1)
					continue;
				
				Vector vBonePos1;
				if (D::WTSVec(Vector(pBoneToWorldOut[i][0][3], pBoneToWorldOut[i][1][3], pBoneToWorldOut[i][2][3]), vBonePos1))
					continue;

				Vector vBonePos2;
				if (D::WTSVec(Vector(pBoneToWorldOut[pBone->iParent][0][3], pBoneToWorldOut[pBone->iParent][1][3], pBoneToWorldOut[pBone->iParent][2][3]), vBonePos2))
					continue;

				CreateLine((int)vBonePos1.x, (int)vBonePos1.y, (int)vBonePos2.x, (int)vBonePos2.y, Color::FromHSB(0, 0, 100), 1.0f);
			}
		}
	}
}
