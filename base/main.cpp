...
DWORD WINAPI OnDllAttach(LPVOID lpParameter)
{
	try
	{
		...
		#ifdef DEBUG_CONSOLE
		// console logging
		if (!L::Attach(XorStr("fl0whaxx developer-mode")))
		...
		L::ofsFile.open(C::GetWorkingPath().append(XorStr("fl0whaxx.log")), std::ios::out | std::ios::trunc);
		#endif
		 ...
		 */
		if (!CNetvarManager::Get().Setup(XorStr("netvars.fl0w")))
		...
		if (!C::Setup(XorStr("default.fl0w")))
		{
		...
		L::Print(XorStr("fl0whaxx successfully loaded"));
		L::PopConsoleColor();
		I::GameConsole->Clear();
		I::ConVar->ConsoleColorPrintf(Color(255, 50, 255, 255), XorStr("fl0whaxx successfully loaded.\nbuild date: %s / %s\n"), __DATE__, __TIME__);
	}
	catch (const std::exception& ex)
	{
		// show error message (or replace to your exception handler)
		MessageBox(nullptr, ex.what(), XorStr("fl0whaxx (error)"), MB_OK | MB_ICONERROR | MB_TOPMOST);
